...
@interface EERegistrationsRequest ()
@property (strong, nonatomic) NSString* params;
@end

@implementation EERegistrationsRequest
...
- (id)initWithQueryParams:(NSString *)params
         ...
         startImmediately:(BOOL)startImmediately
               completion:(EERestAPICompletion)completion
{
	self = [super initWithEndpoint:@"espresso-api/v1/registrations"
                        sessionKey:sessionKey
                               URL:URL
                        completion:completion];
...
- (NSError *)processResultsForError:(NSDictionary *)resultsDict
{
    ...
    NSArray* jsonRegistrationObjects = self.results[EEJSONBodyKey][EEJSONRegistrationsKey];
    ...
    NSMutableArray* registrations = [NSMutableArray new];
    
    for ( NSMutableDictionary* registrationDict in jsonRegistrationObjects )
    {
        EERegistration* registration = [EERegistration registrationWithJSONDictionary:registrationDict];
        [registrations addObject:registration];
    }
    
    _returnedRegistrations = registrations;
