...
@interface EECheckInRequest ()
@property (strong, nonatomic) NSNumber* registrationID;
@end

@implementation EECheckInRequest

#pragma mark - Properties

@synthesize ignorePaymentStatus     = _ignorePaymentStatus;
...
- (id)initWithRegistrationID:(NSNumber *)registrationID
         ...
         ignorePaymentStatus:(BOOL)ignorePaymentStatus
            ...
            startImmediately:(BOOL)startImmediately
                  completion:(EERestAPICompletion)completion
{
	self = [super initWithEndpoint:[NSString stringWithFormat:@"espresso-api/v1/registrations/%@/checkin", registrationID]
                        sessionKey:sessionKey
                               URL:URL
                        completion:completion];
	...
	if ( self )
	{
        ...
        self.ignorePaymentStatus = ignorePaymentStatus;
                     ...
                     completion:(EERestAPICompletion)completion
{
	return [[[self class] alloc] initWithRegistrationID:registrationID
                                               quantity:quantity
                                    ignorePaymentStatus:ignorePaymentStatus
                                             sessionKey:sessionKey
                                                    URL:URL
...
- (void)start
{
    ...
    if ( self.ignorePaymentStatus )
    {
        endpointURL = [endpointURL stringByAppendingString:@"&ignore_payment=true"];
    }
